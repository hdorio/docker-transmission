...
if [ "$1" = 'transmission-daemon' ]; then
  if [ ! -f $settings ]; then
    ...
    if [ "$RPC_HOST_WHITELIST" ];             then jq 'setpath(["rpc-host-whitelist"];             "'$RPC_HOST_WHITELIST'")'           $tmp > /tmp/output.json && mv /tmp/output.json $tmp ; fi
    if [ "$RPC_HOST_WHITELIST_ENABLED" ];     then jq 'setpath(["rpc-host-whitelist-enabled"];     '$RPC_HOST_WHITELIST_ENABLED')'     $tmp > /tmp/output.json && mv /tmp/output.json $tmp ; fi
  ...
  exec gosu transmission:transmission "$@"
fi

exec "$@"
# transmission-daemon -f -g /etc/transmission-daemon/
